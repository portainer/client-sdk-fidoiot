...
if [[ $# -ne 2 ]]; then
    ...
    error "Usage: ${0} <DNS LIST> <IP LIST>"
    error "Example: ${0} '\"fdo-rdv.portainer.io\" \"fdo-manufacturer.portainer.io\" \"fdo-owner.portainer.io\"' '\"7.7.7.7\" \"7.77.77.77\"'"
    error "Make sure to respect the format specified in the example above."
    exit 1
fi
