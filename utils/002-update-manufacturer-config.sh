...
if [[ $# -ne 4 ]]; then
  ...
  error "Usage: ${0} <MANUFACTURER_URL> <API_USER> <API_PASSWORD> <RDVINFO>"
  error "Example: ${0} http://fdo-manufacturer.portainer.io:8039 apiUser apiPassword 81848203191F68820C0182024480C7AE0E8204191F69"
  exit 1
fi

RVINFO=${4}
info "Updating RVINFO with ${RVINFO}"

MANUFACTURER=${1}
API_USER=${2}
API_PASSWORD=${3}

http -A digest -a ${API_USER}:${API_PASSWORD} ${MANUFACTURER}/api/v1/vouchers/${SERIAL} > data/${SERIAL}.bin

echo -n "${RVINFO}" | http -A digest -a ${API_USER}:${API_PASSWORD} ${MANUFACTURER}/api/v1/rvinfo "Content-Type:text/plain; charset=us-ascii"
